...
const GRIMOIRE = [
  ...
  {
    name: 'Scroll-Parallax Descent',
    summary: 'A pinned, Lenis-smoothed fall: a falling video, a tumbling subject, and depth strata that rise and rush past, with a WebGL light atmosphere screened over it.',
    build: {
      kind: 'build',
      label: 'Incantation · build',
      text: `Build a full-bleed, scroll-pinned "descent" — the camera (and a falling subject) plunge down a shaft while parallax strata rise past — in vanilla JS using Lenis for smooth scroll + three.js for the atmosphere. SMOOTH SCROLL (global, once): import Lenis and 'lenis/dist/lenis.css'; if matchMedia('(prefers-reduced-motion: reduce)').matches do NOT init (native scroll); else new Lenis({autoRaf:true, duration:~1.05, smoothWheel:true, anchors:true}) so in-page nav links glide and every getBoundingClientRect()/'scroll' listener keeps working on the eased offset; destroy()/recreate on a reduced-motion flip. STRUCTURE: a centred section head (kicker + title with one italic <em> accent word + lead) ABOVE a tall .track (height 620vh inline — more vh = a longer, slower fall) containing a position:sticky, top:0, height:100svh, overflow:hidden, isolation:isolate stage that runs EDGE-TO-EDGE (no overflow:hidden on any ancestor, or the sticky breaks). Stack the stage back→front with z-index: (0) a CSS void gradient on the stage itself (surface tone → near-black); (1) a <video muted loop playsinline preload="auto"> of the fall {{FALL_VIDEO}}, object-fit:cover, fading in on 'loadeddata' (gradient shows if it 404s); (2) a CSS depth-scrim — a steady vignette plus a bottom darkness whose alpha is calc(base + k*var(--p)); (3) a WebGL <canvas> (see ATMOSPHERE); (4) a .field of parallax sigils; (5) the falling subject <img> {{SUBJECT_PNG}}; (6) a .beats narration band; (7) an optional depth rail. ONE PROGRESS drives all of it: p = clamp(-trackRect.top / (trackRect.height - innerHeight), 0, 1); write it to the stage as a CSS var --p each frame for the scrim/rail. FALL VELOCITY (the "rush"): each frame inst = clamp(abs(scrollY - prevY)/(innerHeight*0.45), 0, 1); vel = lerp(vel, inst, 0.18) — because Lenis eases scrollY, this is the smoothed speed; feed it to the shader and to the subject's lean, and nudge video.playbackRate = clamp(1 + vel*1.4, 1, 2.4). SIGIL STRATA (the "things appearing", as DOM SVG glyphs so they stay crisp): each has {x (0..1), t0 (the progress it crosses centre), depth (0..1 → size/speed/sharpness), label?}. Per frame, rel = p - t0; span = 0.15 + (1-depth)*0.12; if |rel|/span ≥ 1 hide it; else translateY = -rel * innerHeight * (0.6 + depth*1.5) (near layers travel further/faster → parallax), scale = 0.45 + depth*1.05, opacity = (1 - dist²)*(0.32 + 0.68*depth), blur = (1-depth)*4 + dist*2.5 px (depth-of-field), and a vel-driven vertical stretch; set transform: translate(-50%,-50%) translateY(...) rotate(spin) scale(sx, sy). Name the strata to tell the story (e.g. the craft: foundations, components, state, systems, architecture). SUBJECT: keep it ~centred (the camera falls with it) and animate a slow tumble in place — sway = sin(p·4π), bob = sin(time·0.9), rotate = sway*7 + vel*9, plus a small vel scale/stretch. BEATS: short lines that fade in/out — o = clamp(1 - |p - t0|/0.12, 0, 1), smoothstep it for opacity and a small translateY rise. ATMOSPHERE (three.js, PURE ADDITIVE LIGHT on a BLACK canvas, CSS mix-blend-mode:screen so black = transparent and the light blooms over the video): WebGLRenderer({antialias:false, alpha:false, powerPreference:'low-power'}), setClearColor(0x000000,1); OrthographicCamera(-1,1,1,-1,0,1) + PlaneGeometry(2,2) + ShaderMaterial({depthTest:false, depthWrite:false}); uniforms uTime, uResolution, uProgress, uVelocity, and brand-accent colours. The fragment sums: god-rays splaying from a source just above the top (atan around the downward axis × fbm, fading with distance and with (1 - 0.7*uProgress) so the surface light recedes as you fall); a sparse ember/dust field on a hashed grid drifting UP, its cells elongated by /(1+vel·5) into streaks; a slow teal depth-haze (fbm) through the middle; and velocity light-smears (per-column noise × a fast y-scrolling noise × vel). Output gl_FragColor = vec4(clamp(col,0.,1.5), 1.0). CAPABILITY GATES: prefers-reduced-motion → a static composite (a poster still {{POSTER_IMG}} as the stage background + the final beat, no track height, no video, no loop); no WebGL OR !CSS.supports('mix-blend-mode','screen') → simply skip the canvas (the DOM scene still reads). PLUMBING: cap DPR at 2 (1.5 on coarse pointers) and thin the sigil set on coarse pointers; an IntersectionObserver(rootMargin:'128px 0px') + 'visibilitychange' run the rAF loop AND play/pause the video only in view (zero work off-screen; the loop owns ALL writes); a ResizeObserver re-sizes the renderer; on 'webglcontextlost' preventDefault and stop painting; teardown cancels the rAF, disconnects observers, removes listeners, video.removeAttribute('src')+load(), and disposes geometry/material/renderer + forceContextLoss(). Pull every colour, gradient, type ramp and accent glow from the project design system ({{DESIGN_SYSTEM}}) as tokens, and write the head + beats in the site voice from {{BRAND_GUIDELINES}}.`,
    },
    assets: [
      {
        kind: 'video',
        label: 'Conjure · falling background video',
        text: `A vertical falling-POV background video, PORTRAIT 9:16 (e.g. 1080×1920), ~10–14s, that LOOPS SEAMLESSLY (first and last frames identical with matched motion — no visible jump). The camera plunges straight DOWN a {{SHAFT}} (e.g. a deep cavern / mineshaft / vertical tunnel) so walls, particles and light streak continuously UPWARD past the lens; steady, monotonic descent — no cuts, no horizontal camera moves, no on-screen text. Keep the central ~55% relatively uncluttered and mid-to-dark in value (a subject and narration sit there over it) and the motion even (it is also slightly sped up on fast scroll). Graded strictly to the brand palette {{BRAND_COLORS}}; mood: {{MOOD}}. 24–30 fps, ≤10 MB, H.264, faststart enabled.`,
      },
      {
        kind: 'image',
        label: 'Conjure · falling subject (transparent PNG)',
        text: `A single full-body {{SUBJECT}} captured MID-FALL — plummeting downward, body tilted, arms flung outward, robes/cape/hair billowing UPWARD as rushing air lifts them, conveying a weightless plunge. Themed and coloured to {{SITE_THEME}} / brand palette {{BRAND_COLORS}}, {{SITE_STYLE}} rendering, with warm accent glints. The figure must be complete (nothing cropped), centred, and ISOLATED on a PLAIN SOLID WHITE (#FFFFFF) background — flat even white, no scenery, no floor, no cast shadow — so it keys cleanly to transparency. Generate ~1024×1536, then flood-fill the white to alpha and autocrop to a transparent PNG. (The app adds the glow and the tumble.)`,
      },
      {
        kind: 'image',
        label: 'Conjure · poster (reduced-motion still)',
        text: `One tall portrait still (1024×1536, JPEG ~q82) of the whole descent for the reduced-motion fallback and pre-video backdrop: the {{SUBJECT}} plummeting down a vast dark {{SHAFT}} seen from above, shafts of warm light raining from a distant surface overhead, glowing sigils/runes floating at many depths, embers streaming upward. Strong vertigo and depth, graded to {{BRAND_COLORS}}, atmospheric and moody, no text, no border.`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    summary: 'A floating, sci-fi HUD: a WebGL particle core orbited by glassmorphism panels that parallax like a camera and assemble on scroll.',
    build: {
      kind: 'build',
      label: 'Incantation · build',
      text: `Build an immersive "holographic console" section — an abstract WebGL void with a fluid particle core, orbited by floating glassmorphism panels that bank like a camera to the cursor and assemble into formation on scroll — in vanilla JS + three.js. NO art assets: every visual is procedural (particles + CSS glass), so there is nothing to generate. STRUCTURE: a normal centred section head (kicker + title with one italic <em> accent word + lead) above an .obs-stage — a dark, rounded, overflow:hidden, isolation:isolate viewport (height clamp(520px, 82svh, 820px)). Stack it back→front by z-index: (0) an always-on CSS base glow (a big radial-gradient "orb" + a conic-gradient masked "ring", both slowly animated; their brightness reads a --energy custom prop and they carry the whole scene when WebGL is absent); (1) a three.js <canvas> of ADDITIVE light; (2) a HUD frame — four corner brackets, a slow-spinning SVG reticle, a blinking "online" status; (3) an .obs-field of floating glass panels. THE WEBGL SCENE (all PURE ADDITIVE LIGHT on a TRANSPARENT canvas so the CSS void shows through and overlaps bloom to white): WebGLRenderer({antialias:false, alpha:true, powerPreference:'low-power'}), setClearColor(0x000000, 0), outputColorSpace = SRGBColorSpace; a PerspectiveCamera(45°, z≈6.6); a "world" THREE.Group you bank toward the pointer. Add, all with material {transparent:true, depthTest:false, depthWrite:false, blending:THREE.AdditiveBlending}: (a) a CORE — THREE.Points on a fibonacci sphere (≈20k points, 9k on coarse pointers) with a per-point aSeed attribute and a ShaderMaterial whose VERTEX shader displaces each point's radius by 3D fbm value-noise of (dir·freq + time) in two counter-drifting octaves (fluid, breathing morph), mixes from a scattered outer shell to the sphere by a uAssemble uniform (0→1), and sets gl_PointSize = uSize·uDpr·seed·(1/-mvPosition.z)·mix(0.35,1,uAssemble); the FRAGMENT draws a soft round disc (discard if dot(pc,pc)>1) coloured across a neon teal→gold→ember gradient by latitude + displacement, with a fine sin() twinkle; (b) a faint additive WireframeGeometry(IcosahedronGeometry) shell inside the core; (c) DUST — two THREE.Points shells (a fine twinkling starfield that stays in the scene as a stable backdrop, and a sparse large-sprite nebula parented to the world) sharing one cheap drift+twinkle shader; (d) ORReRY — 2–3 thin THREE.LineLoop circles at varied tilts, additive, slowly rotating. INTERACTION + MOTION (one rAF loop owns ALL writes; zero work off-screen): on 'pointermove' over the stage store a target (nx,ny) ∈ [-1,1] from the pointer's fraction of the stage rect; on coarse/no-hover pointers auto-drift the target on sin()/cos() instead so it stays alive. Each frame ease current→target (lerp ≈0.06); compute an "assemble" value from the stage's position (1 when the stage is centred in the viewport, →0 near the edges) and ease it too; derive --energy from assemble + pointer engagement. Then: bank world.rotation.y = time·0.04 + nx·0.5 and .x = -ny·0.32 (plus self-spin on the core/shell/rings); dolly camera.position.z = mix(6.9, 4.7, assemble) with a small (nx,ny) offset and lookAt(0,0,0); write uTime/uAssemble/uEnergy to the core. PARALLAX PANELS (the "floating interfaces", DOM for crisp text): each panel has {x,y (fraction of the stage), depth (0..1)}; every frame set transform: translate(-50%,-50%) translate3d(-nx·THROW·depth, -ny·THROW·depth, 0) rotateX(-ny·5·depth) rotateY(nx·6·depth) scale(...) — near panels (depth→1) throw further; blend each panel in from an outward-scattered offset along its radial-from-centre as assemble→1 (with a slight per-depth stagger) and drive its opacity from the same value; put perspective on .obs-field so the rotations read as 3D. STYLE the panels as glassmorphism: a translucent gradient fill, a 1px neon border, inner+outer glow, backdrop-filter: blur(9px) saturate(1.35) (with an @supports fallback to an opaque fill), a neon top-edge hairline, a drifting diagonal "sheen" pseudo-element, and a hover/focus-within state that pops scale (use the standalone CSS \`scale:\` property so it composes with the JS-written \`transform\`), brightens and intensifies the glow. Each panel shows a glyph, a label + italic sub-label, a small readout and a tiny CSS-animated micro-viz (equalizer bars / a travelling wave / an orbit dot / pulsing dots). CAPABILITY GATES: prefers-reduced-motion → a settled static composite (panels in place at full opacity, CSS core at fixed energy, no canvas, no loop, no parallax) and rebuild on a reduced-motion change; no WebGL (try getContext('webgl2')||'webgl') → skip the canvas, add a .no-gl class, keep the CSS core + DOM parallax. PLUMBING: cap DPR at 2 (1.5 on coarse); an IntersectionObserver(rootMargin:'200px 0px') + 'visibilitychange' run the loop only in view; a ResizeObserver re-sizes the renderer + updates the camera aspect + uDpr; on 'webglcontextlost' preventDefault and stop painting; teardown cancels the rAF, disconnects observers, removes listeners, and disposes every geometry/material/renderer + forceContextLoss(). Pull the whole neon palette from the project design system ({{DESIGN_SYSTEM}}) — take the brand accents and push them to hologram brightness — name the panels after the disciplines of {{SUBJECT_DOMAIN}}, and write the head in the site voice from {{BRAND_GUIDELINES}}.`,
    },
    assets: [],
  },
  {
